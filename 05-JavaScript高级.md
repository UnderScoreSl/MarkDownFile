...
- 解析执行：（解释一行执行一行，多次编译）轻量级解释型的，或是 JIT 编译型的程序设计语言
...
浏览器--DNS服务器（解析ip）--服务器地址

浏览器组成：用户界面，浏览器引擎（查询操作渲染引擎），渲染引擎显示网页内容等等（网络模块，JavaScript解析器，系统功能模块）

#### JavaScript执行过程

全局预解析（所有变量与函数声明都会提前：同名的函数和变量函数优先级高）；

函数内部预解析（所有变量，函数和形参都参与预解析）



**先预解析全局作用域，然后执行全局作用域的代码，在执行全局作用域的代码过程中遇到函数调用就会预解析，然后再执行函数内代码。**



#### JavaScript 的组成

一方面是语法规范，另一方面是web-APIs（bom和dom）

| 组成部分   | 说明                                                         |
| ---------- | ------------------------------------------------------------ |
| ECMAscript | 描述了该语言的语法和基本对象（变量，流程控制语句，数组，函数，作用域，预解析，对象，属性方法，基本类型复杂类型等等，内置对象） |
| BOM        | 浏览器对象模型：描述了与浏览器进行交互的方法和接口（onload页面加载，window顶级对象，定时器，location，history） |
| DOM        | 文档对象模型：描述了处理网页内容的方法和接口（获取页面元素，注册事件，属性样式操作，节点，动态创建元素，事件阶段等等） |
...
例如给数组增加偶数求和方法，但是不允许直接修改对象，只能增加
