...
one：html +css7天    9 天 传统布局    +  3天 html 5 +css3 

two： js     移动web       php + 项目  15      node      vue     框架  微信 ...  大前端    



# 开发工具

我们主要用的 开发工具有   chrome  、  sublime 、  photoshop、vscode、webstorm



 

## Photoshop(协助切图，查看rgb等)
...
```
浏览器是网页运行的平台，常用的浏览器有IE、火狐（Firefox）、谷歌（Chrome）、Safari和Opera等。我们平时称为五大浏览器。微软Edge
```
...
target：用于指定链接页面的打开方式，其取值有```_self```和``` _blank``` 两种，其中 _self_ 为默认值，_blank_ 为在新窗口中打开方式。
...
3.如果当时没有确定链接目标时，通常将链接标签的href属性值(即href="#")，表示该链接暂时为一个空链接。
